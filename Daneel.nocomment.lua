...
function table.getlength( t, keyType )
    ...
    for key, value in pairs( t ) do
        if 
            keyType == nil or
            type( key ) == keyType or
            tostring( Daneel.Debug.GetType( key ) ):lower() == keyType 
        then
            length = length + 1
        end
    end
...
Daneel.Debug.tryGameObject = nil 
...
function Daneel.Event.Fire( object, eventName, ... )
    ...
    for i, listener in ipairs( listeners ) do
        ...
        if listenerType == "function" or listenerType == "userdata" then
            ...
            if listener.isDestroyed ~= true or (mt == GameObject and listener.inner ~= nil)  then 
...
function TextRenderer.SetAlignment(textRenderer, alignment)
    ...
    Daneel.Debug.CheckArgType(textRenderer, "textRenderer", "TextRenderer", errorHead)
    local argType = Daneel.Debug.CheckArgType(alignment, "alignment", {"string", "userdata", "number"}, errorHead) 
    if argType == "string" then
...
function CraftStudio.Destroy( object )
    ...
    if type( object ) == "table" then
        ...
        Daneel.Event.StopListen( object ) 
...
function GameObject.__newindex( gameObject, key, value )
    ...
    if key ~= ucKey and key ~= "transform" then 
        local funcName = "Set" .. ucKey
        if GameObject[ funcName ] ~= nil then
...
function GameObject.New( name, params )
    ...
    local scene = Asset.Get( name, "Scene" ) 
    if scene ~= nil then
...
function GameObject.BroadcastMessage(gameObject, functionName, data)
    ...
    Daneel.Debug.CheckArgType(functionName, "functionName", "string", errorHead)
    Daneel.Debug.CheckOptionalArgType(data, "data", "table", errorHead)
    local allGos = gameObject:GetChildren(true, true) 
...
function GameObject.GetComponent( gameObject, componentType )
    ...
    if component == nil then
        ...
        elseif Daneel.Config.componentObjects[ componentType ] == nil then 
            local script = Asset.Get( componentType, "Script", true ) 
...
function GameObject.Destroy( gameObject )
    ...
    Daneel.Debug.CheckArgType( gameObject, "gameObject", "GameObject", errorHead )
    for i, go in pairs( gameObject:GetChildren( true, true ) ) do 
        go:RemoveTag()
    end
    for key, value in pairs( gameObject ) do
        if key ~= "inner" and type( value ) == "table" then 
...
function Daneel.Load()
    ...
    if table.getvalue( _G, "DaneelUserConfig" ) ~= nil and type( DaneelUserConfig ) == "function" then 
        Daneel.Config = table.deepmerge( Daneel.Config, DaneelUserConfig() ) 
    end
    ...
    if type( Camera.ProjectionMode.Orthographic ) == "number" then 
        CS.IsWebPlayer = true
    end
...
function Daneel.LateLoad( source )
    ...
    go:CreateScriptedBehavior( DaneelScriptAsset ) 
end
local luaDocStop = ""
