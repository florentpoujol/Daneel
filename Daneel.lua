...
function Daneel.DefaultConfig()
    ...
    config.objects = table.merge( config.objects, config.assetObjects )
...
Daneel.Event.Listen( "OnDaneelLoad", function()
    -- ScriptAlias
    for alias, path in pairs( Daneel.Config.scriptPaths ) do
        ...
        if script ~= nil then
            ...
            if Daneel.Config.debug.enableDebug then
                print( "CraftStudio.Load() : WARNING : item with key '" .. alias .. "' and value '" .. path .. "' in 'Daneel.Config.scriptPaths' is not a valid script path." )
            end
        end
    end
...
function GameObject.AddComponent( gameObject, componentType, params )
    ...
    local component = nil
    if table.containsvalue( Daneel.Config.componentTypes, componentType ) then
...
function GameObject.GetComponent( gameObject, componentType )
    ...
    if component == nil and table.containsvalue( Daneel.Config.componentTypes, componentType ) then
